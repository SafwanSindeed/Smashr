...
export default function GPNConnect() {
  ...
  try {
    ...
    const API = `https://www.globalpickleball.network/component/api?apiCall=registerUser&format=raw&devKey=264784-q4jMNhO3X
    ...
    try {
      ...
      Alert.alert(
        "GPN Error",
        "Unexpected response from server. Check console."
      );
    ...
    const gpnUsername = data.username ?? data.user_name ?? data.login ?? null;
    ...
    const safeUsername = gpnUsername ?? username; // fallback to input

    // 🔥 enforce string here too
    const safeId = gpnId ? String(gpnId) : null;
    ...
    try {
      ...
      Alert.alert("Database Error", "GPN created but failed to save locally.");
    }
